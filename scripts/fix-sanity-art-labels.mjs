...
import { structureArtLabelsInBlocks } from "./lib/artLabelText.mjs";
...
async function main() {
  ...
  for (const row of rows) {
    ...
    const structured = structureArtLabelsInBlocks(row.body);
    if (!structured || !blocksChanged(row.body, structured)) continue;

    console.log(`→ ${row._type} / ${row.slug}`);
    if (!dryRun) {
      await client.patch(row._id).set({ body: structured }).commit();
