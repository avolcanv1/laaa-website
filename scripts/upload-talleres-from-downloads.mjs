/**
 * Sube talleres desde carpetas locales (p. ej. ~/Downloads/03).
 * Ignora subcarpetas llamadas "unused".
 *
 *   node scripts/upload-talleres-from-downloads.mjs [--root PATH] [--only 03-01] [--dry-run]
 */
...
const DOC_TYPE = "taller";
const DEFAULT_ROOT = join(homedir(), "Downloads", "03");

const IMAGE_EXT = new Set([".jpg", ".jpeg", ".png", ".webp", ".gif"]);

/** Carpeta → slug en Sanity (coherente con talleresContent.ts). */
const FOLDER_SLUG = {
  "03-01": "pensando-en-vacio",
  "03-02": "thinking-by-modelling",
  "03-03": "no-tocar-es-preservar",
  "03-04": "digitalizacion-colectiva",
  "03-05": "codice-colectivo",
};

/** Fechas corregidas cuando el .txt trae un valor ambiguo o erróneo. */
const LIST_DATE_OVERRIDE = {
  "03-01": "2023.09.01",
};
...
function parseListDate(raw) {
  ...
  const rangeDmy = s.match(/^(\d{1,2})(?:-\d{1,2})?\.(\d{1,2})\.(\d{4})$/);
  if (rangeDmy) {
    const [, d, m, y] = rangeDmy;
...
function parseMetadataTxt(txtPath) {
  ...
  return { fecha: meta.fecha, title, bodyRaw };
}
...
function resolveSlug(folderName) {
  const num = folderName.match(/^(03-\d{2})/)?.[1];
...
async function uploadProject(client, project, { dryRun, assetCache }) {
  ...
  const listDate =
    LIST_DATE_OVERRIDE[folderName.match(/^(03-\d{2})/)?.[1] ?? ""] ??
    parseListDate(meta.fecha);
  ...
  if (images.length > 0) process.stdout.write("\n");
...
async function main() {
  ...
  const projects = discoverProjects(root).filter((p) => {
    if (!only) return true;
    const num = p.folderName.match(/^(03-\d{2})/)?.[1];
