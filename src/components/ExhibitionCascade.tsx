...
function ExhibitionCascadeFullHit({
  ...
  onOpenLightbox: (slideIndex: number) => void;
  priority?: boolean;
  /** Trailing odd image — portrait stays one grid column like pair cells */
  isLast: boolean;
}) {
  ...
  const singleColumn = isLast && !landscape;
  ...
  return (
    <button
      ...
      className={[
        "exhibitionCascade__hit",
        singleColumn ? "" : "exhibitionCascade__hit--full",
    ...
    >
      <CascadeArchivalMedia
        src={src}
        className={
          singleColumn ? "exhibitionCascade__cell" : "exhibitionCascade__full"
        }
        priority={priority}
